...
describe('Family API:', function() {
  describe('GET /api/families', function() {
    var familys;
  ...
  describe('GET /api/families/:id', function() {
    var family;

    beforeEach(function(done) {
      request(app)
        .get(`/api/families/${newFamily._id}`)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          if(err) {
  ...
  describe('PUT /api/families/:id', function() {
    var updatedFamily;

    beforeEach(function(done) {
      request(app)
        .put(`/api/families/${newFamily._id}`)
        .send({
          name: 'Updated Family',
    ...
    it('should respond with the original family', function() {
      expect(updatedFamily.name).to.equal('New Family');
      expect(updatedFamily.info).to.equal('This is the brand new family!!!');
    });
  ...
  describe('PATCH /api/families/:id', function() {
    var patchedFamily;

    beforeEach(function(done) {
      request(app)
        .patch(`/api/families/${newFamily._id}`)
        .send([
  ...
  describe('DELETE /api/families/:id', function() {
    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete(`/api/families/${newFamily._id}`)
        .expect(204)
        .end(err => {
          if(err) {
            return done(err);
